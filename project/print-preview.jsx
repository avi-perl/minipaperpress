// Print preview modal — composes the 8.5×11 sheet(s) with as many copies as fit.
const { useMemo: ppMemo, useState: ppState, useEffect: ppEffect, useRef: ppRef } = React;

const SHEET_W_IN = 8.5;
const SHEET_H_IN = 11;

function PrintPreview({ project, onClose }) {
  const { pageW, pageH, folds, frontHtml, backHtml, unit } = project;
  const [margin, setMargin] = ppState(0.25); // inches
  const [gap, setGap] = ppState(0.15);
  const [showFoldArrows, setShowFoldArrows] = ppState(true);
  const [showCutBorder, setShowCutBorder] = ppState(true);
  const [shareOpen, setShareOpen] = ppState(false);
  const [pdfHintOpen, setPdfHintOpen] = ppState(false);
  ...
  const stageRef = ppRef(null);
  const [ppi, setPpi] = ppState(40);
  ...
  const handleSavePdf = () => {
    setPdfHintOpen(true);
    setTimeout(() => { window.print(); setPdfHintOpen(false); }, 350);
  };
  ...
  return (
    <div className="print-page">
      ...
      <div className="print-body">
        ...
        <div className="print-side no-print">
          ...
          <div className="action-list">
            <ActionButton
              icon={<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><path d="M14 2v6h6"/><path d="M9 14h6"/><path d="M9 18h6"/></svg>}
              title="Save as PDF"
            ...
            <ActionButton
              icon={<Icon.Print/>}
              title="Print"
              subtitle="Send to your printer"
              primary
              onClick={handlePrint}
            />
            <ActionButton
              icon={<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="18" cy="5" r="3"/><circle cx="6" cy="12" r="3"/><circle cx="18" cy="19" r="3"/><line x1="8.6" y1="13.5" x2="15.4" y2="17.5"/><line x1="15.4" y1="6.5" x2="8.6" y2="10.5"/></svg>}
          ...
          <SliderRow label="Margin" value={margin} min={0} max={0.75} step={0.05}
            onChange={setMargin} format={(v) => round2(v) + "″"} />
          <SliderRow label="Gap" value={gap} min={0} max={0.5} step={0.05}
            onChange={setGap} format={(v) => round2(v) + "″"} />
        ...
        <div className="print-stage" ref={stageRef}>
          <div className="sheets-row">
            <SheetCol label="Front" side="front" project={project} layout={layout} ppi={ppi}
              showCutBorder={showCutBorder} showFoldArrows={showFoldArrows} />
            <SheetCol label="Back" side="back" project={project} layout={layout} ppi={ppi}
              showCutBorder={showCutBorder} showFoldArrows={showFoldArrows} />
          </div>
        </div>
      </div>
      ...
      {shareOpen && (
        <ShareDialog project={project} onClose={() => setShareOpen(false)} />
      )}
      {pdfHintOpen && (
...
function ShareDialog({ project, onClose }) {
  ...
  const [copied, setCopied] = ppState(false);
...
function SheetCol({ label, side, project, layout, ppi, showCutBorder, showFoldArrows }) {
  return (
    <div className="sheet-col">
      <div className="sheet-label">{label} · 8.5 × 11 in</div>
      <Sheet side={side} project={project} layout={layout} ppi={ppi}
        showCutBorder={showCutBorder} showFoldArrows={showFoldArrows} />
    </div>
  );
}

function Sheet({ side, project, layout, ppi, showCutBorder, showFoldArrows }) {
  const { pageW, pageH, folds, frontHtml, backHtml } = project;
  const html = side === "front" ? frontHtml : backHtml;

  if (layout.count === 0) {
    return (
      <div className="sheet" style={{ "--ppi": ppi }}>
        <div className="sheet-empty">
          Nothing to print — card is too large for the page.
        </div>
      </div>
    );
  }
...
function SheetCell({ rotated, xIn, yIn, wIn, hIn, pageW, pageH, folds, html, showCutBorder, showFoldArrows, neighbors }) {
  // All measurements stay in inches; converted to display px via `--ppi`.
  const px = (inches) => `calc(${inches} * var(--ppi) * 1px)`;

  // Outer cell — sized in inches (display via --ppi).
  const cellStyle = {
    position: "absolute",
    left: px(xIn), top: px(yIn), width: px(wIn), height: px(hIn),
  ...
  return (
    <div style={cellStyle}>
      <div style={cardFrame}>
        ...
        {showFoldArrows && folds.map((f) => (
          <FoldArrowPair key={"a-" + f.id} fold={f}
            cardW={pageW} cardH={pageH} natural={NATURAL}
            rotated={rotated} neighbors={neighbors} />
        ))}
      </div>
    </div>
  );
...
function FoldArrowPair({ fold, cardW, cardH, natural, rotated, neighbors }) {
  const offsetIn = 0.06;
  const isH = fold.axis === "h";
  const cellEdgeFor = (cardEdge) => {
    if (!rotated) return cardEdge;
    return { left: "top", right: "bottom", top: "right", bottom: "left" }[cardEdge];
  };
  const hideOn = (cardEdge) => neighbors && neighbors[cellEdgeFor(cardEdge)];
  const npx = (inches) => inches * natural;
...
function ArrowHead({ direction, left, top }) {
  const size = 8;
  const pts = {
  ...
  const style = {
    ...
    transform:
      direction === "right" ? "translate(-100%, -50%)" :
      direction === "left"  ? "translate(0, -50%)" :
      direction === "down"  ? "translate(-50%, -100%)" :
                              "translate(-50%, 0)",
    pointerEvents: "none",
  };
  return (
